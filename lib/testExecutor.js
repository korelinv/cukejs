...
module.exports = function RunTest({test, stepDefenitions, hooks, bundle})
{
    return new Promise(function(resolve, reject) {
        ...
        let errorOut = (error) => {throw error};
        let execute = () => Execute({
            ...
            resolve: () => {
                WaitFor(() => report.testPassed(scope))
                    .then(() => WaitFor(() => report.after(scope)))
                    .then(() => resolve())
                    .catch(errorOut);
            },
            reject: (scope) => {
                WaitFor(() => report.testFailed(scope))
                    .then(() => WaitFor(() => report.after(scope)))
                    .then(() => resolve())
                    .catch(errorOut);
            },
