...
const featureParser = require('./featureParser/featureParser')();
const testBuilder = require('./testBuilder/testBuilder')();
...
module.exports = function BundleRunner({features, options, steps, hooks}, callback)
{
    ...
    options = options || {};
    let instances = options.instances || 1;
    ...
    WaitFor(() => report.before(bundle))
        .then(() => {
            ...
            for (let instance = 1; instances >= instance; instance++)
            {
                runningInstances.push(new Promise(function(resolve, reject) {
                    runNextTest(resolve);
                }));
            };
            return Promise.all(runningInstances)
        })
